...
(function (A) {
  ...
  A.scene('continuity-flow', function (root) {
    ...
    var note = S.text(W / 2, 698,
      'probability drains where the arrows spread apart and gathers where they crowd together',
      's-lbl-b', 'middle');
    note.setAttribute('font-size', '12');
    svg.appendChild(note);
  ...
  A.scene('plane-wave-current', function (root, api) {
    ...
    return function (p, tSec) {
      ...
      nums[6].textContent = 'imaginary part of (1.9.5): ' + J1[1].toFixed(1) + ' — J is real';
      ...
      S.setD(reW, S.polyD(S.sample(320, 0, 1, function (u) {
        return [M.lerp(wx0, wx1, u), wcy - Math.cos(M.TAU * cycles * u - time * 1.6 * (kU >= 0 ? 1 : -1)) * wamp];
      })));
      S.setD(imW, S.polyD(S.sample(320, 0, 1, function (u) {
        return [M.lerp(wx0, wx1, u), wcy - Math.sin(M.TAU * cycles * u - time * 1.6 * (kU >= 0 ? 1 : -1)) * wamp];
      })));

      var drift = (api.reduced ? 0.25 : time * 0.09) * (kU >= 0 ? 1 : -1) * Math.min(2.5, Math.abs(kU) + 0.2);
  ...
  A.scene('total-probability', function (root) {
    ...
    for (ir = 46; ir >= 1; ir--) {
      var rr = RMAX * ir / 46;
      var ring = S.circle(cx, cy, rr * PXR, null);
      ring.setAttribute('fill', 'none');
      ring.setAttribute('stroke', 'var(--probability)');
      ring.setAttribute('stroke-width', String(RMAX / 46 * PXR + 1));
      ring.setAttribute('stroke-opacity', String(0.85 * dens(rr) / dens(0)));
      gBlob.appendChild(ring);
    }

    var gSphere = S.g({});
    svg.appendChild(gSphere);
    var sph = S.circle(cx, cy, PXR, 's-axis s-dash');
    sph.setAttribute('fill', 'none');
    sph.setAttribute('stroke', 'var(--quantum)');
  ...
  A.scene('question-191', function (root) {
    ...
    rows[2].textContent = 'δP/δt     = ' + ((pdt - p00) / DT).toFixed(4);
    rows[3].textContent = '∂P/∂t at (0,0) = ' + dpdt.toFixed(4);
    crows[0].textContent = '∂J/∂x = −∂P/∂t = ' + (-dpdt).toFixed(4);
    crows[1].textContent = 'straight from J:  ' + djdx.toFixed(4);
    crows[2].textContent = 'residual = ' + sci(Math.abs(dpdt + djdx), 1);
    caution.textContent = 'one caution: at the peak of the t = 0 curve the same derivative is ' +
      atPeak.toFixed(1) + ' exactly,';
